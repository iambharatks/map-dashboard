  ...
  function _dc_leaflet(dc) {
    ...
    dc_leaflet.leafletBase = function (_chart) {
      ...
      var _tiles = function (map) {
        L.tileLayer(
          "https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token=pk.eyJ1IjoiaWFtYmhhcmF0a3MiLCJhIjoiY2txcnJ5Y3VjMTJ2bzJwbmJvNGpzYjN3ciJ9.XM5fi31G0lovEyYkhNkgUg",
          {
            attribution:
            ...
            zoomOffset: -1,
            accessToken:
              "pk.eyJ1IjoiaWFtYmhhcmF0a3MiLCJhIjoiY2txcnJ5Y3VjMTJ2bzJwbmJvNGpzYjN3ciJ9.XM5fi31G0lovEyYkhNkgUg",
          }
        ).addTo(map);
      };
      ...
      _chart._doRender = function () {
        ...
        for (var ev in _cachedHandlers) _map.on(ev, _cachedHandlers[ev]);
    ...
    dc_leaflet.legend = function () {
      ...
      _legend.LegendClass = function (LegendClass) {
        if (!arguments.length) return _LegendClass;
    ...
    dc_leaflet.markerChart = function (parent, chartGroup) {
      ...
      var _marker = function (d, map) {
        var marker = new L.Marker(
          _chart.toLocArray(_chart.locationAccessor()(d)),
          {
            ...
            icon: _icon(),
            clickable:
              _chart.renderPopup() || (_chart.brushOn() && !_filterByArea),
            draggable: false,
          }
        );
        return marker;
      ...
      _chart._postRender = function () {
        ...
        if (_cluster) {
          _layerGroup = new L.MarkerClusterGroup(
            _clusterOptions ? _clusterOptions : null
          );
        } else {
      ...
      var zoomFilter = function (e) {
        ...
        if (_filterByArea) {
          var filter;
          if (
            _chart.map().getCenter().equals(_chart.center()) &&
          ...
          (e.type === "click" ||
            (_markerList.indexOf(_chart.filter()) !== -1 &&
              !_chart
                .map()
                .getBounds()
                .contains(_markerList[_chart.filter()].getLatLng())))
        ) {
          dc.events.trigger(function () {
            _chart.filter(null);
            if (_renderPopup) {
      ...
      var doFilterByArea = function (dimension, filters) {
        ...
        if (filters && filters.length > 0) {
          ...
          if (
            !_innerFilter &&
            _chart.map().getBounds().toString !== filters[0].toString()
          ) {
    ...
    dc_leaflet.choroplethChart = function (parent, chartGroup) {
      ...
      var processFeatures = function (feature, layer) {
        ...
        if (v && v.d) {
          ...
          if (_chart.brushOn()) layer.on("click", selectFilter);
        }
      };
